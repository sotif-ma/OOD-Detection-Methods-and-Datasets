...
class AltCLIPClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 pos_topk=None,
                 seed=0):
        super(AltCLIPClassifier, self).__init__(init_cfg)
        self.local_rank = os.environ['LOCAL_RANK']
        self.device = "cuda:{}".format(self.local_rank)

        self.model = AltCLIPModel.from_pretrained("BAAI/AltCLIP").to(self.device)
        self.processor = AutoProcessor.from_pretrained("BAAI/AltCLIP")
        self.model.eval()
        ...
        text_inputs_pos = self.processor(self.id_prompts, padding=True, return_tensors="pt").to(self.device)
        with torch.no_grad():
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(0, len(words_total), emb_batchsize)):
                text_inputs_neg_batch = self.processor(words_total[i: i + emb_batchsize], padding=True, return_tensors="pt").to(self.device)
