...
class OpenClipClassifier(BaseClassifier):
    ...
    def __init__(self,
                 ...
                 pos_topk=None,
                 seed=0):
        super(OpenClipClassifier, self).__init__(init_cfg)
        self.local_rank = os.environ['LOCAL_RANK']
        self.device = "cuda:{}".format(self.local_rank)
        # self.model = open_clip.create_model_and_transforms('ViT-B-32', pretrained='laion2b_s34b_b79k')[0].to(self.device)
        # self.model = open_clip.create_model_and_transforms('ViT-g-14', pretrained='laion2b_s34b_b88k')[0].to(self.device)
        # self.model = open_clip.create_model_and_transforms('ViT-bigG-14', pretrained='laion2b_s39b_b160k')[0].to(self.device)
        if arch == 'EVA02-L-14-336':
            self.model = open_clip.create_model_and_transforms(arch, pretrained='merged2b_s6b_b61k')[0].to(self.device)
        elif arch == 'ViT-L-14-CLIPA-336':
            self.model = open_clip.create_model_and_transforms(arch, pretrained='datacomp1b')[0].to(self.device)
        else:
            raise NotImplementedError
        self.tokenize = open_clip.get_tokenizer(arch)
        self.model.eval()
        ...
        with torch.no_grad():
            self.text_features_pos = []
            for i in tqdm(range(0, len(self.id_prompts), emb_batchsize)):
                text_inputs_pos_batch = self.tokenize(self.id_prompts[i: i + emb_batchsize]).to(self.device)
                x = self.model.encode_text(text_inputs_pos_batch).to(torch.float32)
                self.text_features_pos.append(x)
            self.text_features_pos = torch.cat(self.text_features_pos, dim=0)
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(0, len(words_total), emb_batchsize)):
                text_inputs_neg_batch = torch.cat([self.tokenize(f"{c}") for c in words_total[i: i + emb_batchsize]]).to(self.device)
                x = self.model.encode_text(text_inputs_neg_batch)
